...
class self_net(nn.Module):
    def __init__(self, n_channels=3, n_classes=4, bilinear=True, attention=False, base_c=32):
        super(self_net, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.attention = attention  # 初始化attention属性
        ...
        # 注意力模块（通道数需与各层匹配）
        if self.attention:
            self.att1 = UniversalInvertedBottleneckBlock(base_c, base_c, 0, 3, True, 1, 2)
            self.att2 = UniversalInvertedBottleneckBlock(base_c * 2, base_c * 2, 0, 3, True, 1, 2)
            self.att3 = UniversalInvertedBottleneckBlock(base_c * 4, base_c * 4, 0, 3, True, 1, 2)
            self.att4 = UniversalInvertedBottleneckBlock(base_c * 8, base_c * 8, 0, 3, True, 1, 2)
