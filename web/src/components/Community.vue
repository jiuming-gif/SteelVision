...
const img = (s) => 'https://picsum.photos/seed/' + s + '/400/500'

const tab = ref('feed')
const postSeed = ref(0)
const videoSeed = ref(0)
...
function refreshFeed() {
  postSeed.value++
  const s = postSeed.value
  const now2 = Date.now()
  const h2 = (h) => new Date(now2 - h * 3600000).toISOString()
  const img2 = (seed) => 'https://picsum.photos/seed/' + seed + s + '/400/500'
  const fresh = [
    { id: now2 + 1, title: '汽车凹陷修复工具实测：这3款撬棒最好用', image_url: img2('dent-tools2'), likes: 432 + s * 100, liked: false, author: { username: '凹陷修复小陈' }, created_at: h2(0.1), content: "做凹陷修复3年用过不下20款撬棒推荐3款。日本田岛弹性好不伤漆适合精细操作，德力西套装性价比高新手上路首选。提醒：工具重要但手艺更重要新手先拿废钣金练手！" },
    { id: now2 + 2, title: '钣金喷漆行内黑话大全：新人避坑必看', image_url: img2('slang-guide'), likes: 678 + s * 50, liked: false, author: { username: '修车老司机' }, created_at: h2(0.2), content: "进修理厂听到这些词别懵！鹦鹉漆是高端清漆品牌不是真的鹦鹉。橘皮是漆面像橘子皮喷枪没调好。苍蝇屎是漆面小颗粒。咬底是新旧漆起反应起皱。收藏备用修车时显得你很懂行！" },
    { id: now2 + 3, title: '工业相机选型指南：线扫vs面阵别再选错了', image_url: img2('camera-guide'), likes: 534 + s * 30, liked: false, author: { username: '工业视觉老K' }, created_at: h2(0.3), content: "工业视觉检测中相机选型直接影响检测效果。面阵相机一次拍摄整个画面适合静态检测。线扫相机逐行扫描拼接适合卷材板材连续检测。关键参数：分辨率等于缺陷最小尺寸除以3。" },
    { id: now2 + 4, title: '汽车底盘装甲到底要不要做？修理厂不会说的实话', image_url: img2('undercoat'), likes: 891 + s * 40, liked: false, author: { username: '底盘专家老张' }, created_at: h2(0.4), content: "新车要不要做底盘装甲？建议做：沿海地区盐雾腐蚀、北方冬季融雪剂、经常跑烂路。不建议：城市通勤车原厂防腐足够、干燥地区、3年以上老车。价格参考：材料加工时800到1500。" },
    { id: now2 + 5, title: '工业缺陷检测常用公开数据集横评：NEU到GC10', image_url: img2('datasets'), likes: 432 + s * 20, liked: false, author: { username: '工业视觉老周' }, created_at: h2(0.5), content: "整理了工业缺陷检测常用公开数据集。NEU-DET：6种热轧钢缺陷1800张入门首选。GC10-DET：10种钢缺陷3570张覆盖面更广。KolektorSDD：塑料表面缺陷小样本挑战。经验：公开数据集做预训练可以上线一定要用产线真实样本finetune。" },
  ]
  posts.value = [...fresh, ...posts.value]
}

function loadMorePosts() { refreshFeed() }

function loadMoreVideos() {
  videoSeed.value++
  const s = videoSeed.value
  const now2 = Date.now()
  const h2 = (h) => new Date(now2 - h * 3600000).toISOString()
  const VV = '/web/videos/'
  const vids = [
    { id: now2 + 200 + s, title: '沉浸式发动机积碳清洗解压必看', content: '#发动机清洗 #积碳 #解压', video_url: VV + '04_dent_repair.mp4', likes: 6534 + s * 200, liked: false, playing: false, author: { username: '汽修小陈' }, created_at: h2(0.1) },
    { id: now2 + 201 + s, title: '全自动产线焊接机器人：工业之美', content: '#工业机器人 #焊装 #自动化', video_url: VV + '05_paint_touchup.mp4', likes: 4567 + s * 100, liked: false, playing: false, author: { username: '智造前线' }, created_at: h2(0.2) },
    { id: now2 + 202 + s, title: '3分钟看完一台车从钣金到喷漆全过程', content: '#汽车制造 #涂装 #钣金喷漆', video_url: VV + '06_welding_qa.mp4', likes: 12345 + s * 300, liked: false, playing: false, author: { username: '汽车工厂日记' }, created_at: h2(0.3) },
  ]
  videoPosts.value = [...videoPosts.value, ...vids]
}
...
async function createPost() {
  ...
  try {
    ...
    form.append('title', newPost.value.title)
    form.append('content', newPost.value.content)
    form.append('author_id', '1')
    if (mediaFile.value) {
      form.append(createType.value === 'image' ? 'image_file' : 'video_file', mediaFile.value)
    }
    const { data } = await axios.post('/api/community/post', form)
    const newP = {
      ...data,
      liked: false,
      author: { username: '我' },
      image_url: mediaPreview.value || ('https://picsum.photos/seed/user' + Date.now() + '/400/500'),
    }
    posts.value.unshift(newP)
    if (newP.video_url) videoPosts.value.unshift({ ...newP, playing: false })
...
function onMediaSelect(e) {
  ...
  reader.onload = ev => { mediaPreview.value = ev.target.result }
...
function onVideoLoaded(idx) {
  if (idx === 0) {
    const v = videoRefs.value[0]
    if (v) { v.play().catch(() => {}) }
    videoPosts.value[0].playing = true
  }
}
...
function togglePlay(idx) {
  ...
  if (video.paused) {
    video.play().catch(() => {})
    videoPosts.value[idx].playing = true
  } else {
    video.pause()
    videoPosts.value[idx].playing = false
  }
}

function onVideoScroll() {
  const container = videoFeed.value
  if (!container) return
  const idx = Math.round(container.scrollTop / container.clientHeight)
  if (idx !== currentVideoIdx.value) {
    const oldVid = videoRefs.value[currentVideoIdx.value]
    if (oldVid) { oldVid.pause(); videoPosts.value[currentVideoIdx.value].playing = false }
    currentVideoIdx.value = idx
    const newVid = videoRefs.value[idx]
...
watch(tab, (val) => {
  if (val === 'video') {
    nextTick(() => {
      if (videoPosts.value.length > 0 && videoRefs.value[0]) {
